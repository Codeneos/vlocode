...
describe('vlocityDatapackService', () => {   
    ...
    describe('#resolveProjectPathFor (private)', () => { 
        it("should always return absolute path", function() { 
            let vds = [
                new vlocityDatapackService(null, <VlocodeConfiguration>{ projectPath: '.\\project\\vlc\\' }),
                new vlocityDatapackService(null, <VlocodeConfiguration>{ projectPath: '.\\' }),
                new vlocityDatapackService(null, <VlocodeConfiguration>{ projectPath: '.\\project\\folder\\..' }),
                new vlocityDatapackService(null, <VlocodeConfiguration>{ projectPath: '.\\project' })
            ];
        ...
        it("should return project path as-is when absolute", function() { 
            let vds = new vlocityDatapackService(null, <VlocodeConfiguration>{ projectPath: 'x:\\project\\vlc\\' });
            let uriForResolve = vscode.Uri.file('./myfile');
            // assert
    ...
    describe('#getDatapackManifestKey',  () => {
        it('should resolve single level datapacks', () => {
            ...
            let vds = new vlocityDatapackService(null, <VlocodeConfiguration>{ projectPath: 'c:\\project\\vlc\\' }).getDatapackManifestKey(datapackFile);
            // assert
