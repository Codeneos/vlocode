...
describe('PropertyTransformHandler', () => {
    ...
    describe('transformPropertyProxy', () => {
        ...
        it('nested objects fields should be accessible and wrapped', () => {
            const sut = transformPropertyProxy(testObject, toLowerCase);
            expect(sut.nested.key).equals(testObject.Nested.Key);
        });
        it('objects in nested arrays should be accessible and wrapped', () => {
            const sut = transformPropertyProxy(testObject, toLowerCase);
            expect(sut.array[0].key).equals(testObject.Array[0].Key);
        });
        it('array mutations should apply to target', () => {
            const target = [];
        ...
        it('object mutations existing properties should apply to target', () => {
            const sut = transformPropertyProxy(testObject, toLowerCase);
            sut.mutateme = 'Test1';
