...
type ScriptFilter = string | Record<string, string | undefined>;

export default class OmniScriptActivate extends SalesforceCommand<typeof OmniScriptActivate> {
    ...
    static args = {
        scriptFilter: Args.string({
            description:
                'Salesforce ID or <type>/<subType>(/<language>) filter of the scripts to activate. ' +
                'Supports wildcard characters, i.e: "MACD/" to activate multiple scripts',
            required: false,
        }),
    };

    static flags = {
        'parallel-activations': Flags.integer({
            default: 4,
        ...
        'skip-reactivate-dependencies': Flags.boolean({
            default: false,
            summary: 'skips reactivating parent scripts that embed any of the scripts that are being activated',
            description:
                'When you activate a re-usable OmniScript all the OmniScript that embed this script will also get re-activated and updated.',
        }),
        'remote-activation': Flags.boolean({
            default: false,
            ...
            description:
                ...
                'Enable this when you experience issues or inconsistencies in scripts deployed through Vlocode.',
        }),
        'debug-activation': Flags.boolean({
            default: false,
            summary: 'save the updated script definitions as JSON file',
            description:
    ...
    static examples = [
        ...
        '<%= config.bin %> <%= command.id %> "MACD/" -u my-org',
    ];
    ...
    public async run() {
        ...
        while (true) {
            ...
            await forEachAsyncParallel(scriptsToActivate, async info => {
                if (this.flags['debug-activation']) {
                ...
                try {
                    ...
                    await activator.activate(info.script.id, {
                        toolingApi: !this.flags['use-metadata-api'],
                        skipLwcDeployment: this.flags['skip-lwc'],
                        remoteActivation: this.flags['remote-activation'],
                        reactivateDependentScripts: !!scriptFilter && this.flags['skip-reactivate-dependencies'] !== true
                    });
            ...
            }, this.flags['parallel-activations']);
        }
    ...
    private parseScriptFilter(value: string | undefined): ScriptFilter | undefined {
        if (value === undefined) {
            return undefined;
        }
        ...
        return Object.fromEntries(filter.map((part, index) => [['type', 'subType', 'language'][index], part || undefined]));
    }
    ...
    private async getScripts(scriptFilter: ScriptFilter | undefined): Promise<Map<string, ScriptActivationInfo> | undefined> {
        const lookup = this.container.new(OmniScriptAccess);
        const scripts = await lookup.filter(scriptFilter as any);
