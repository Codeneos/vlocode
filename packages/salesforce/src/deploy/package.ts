...
type SalesforcePackageBufferOptions = {
...
export class SalesforcePackage {
    ...
    /**
     * Creates a new package from a zip buffer generated by {@link getBuffer}.
     * Package files are loaded with a best-effort component mapping.
     * @param packageBuffer Buffer containing the package zip
     */
    public static async fromBuffer(packageBuffer: Buffer): Promise<SalesforcePackage> {
        const archive = await ZipArchive.loadAsync(packageBuffer);
        const packageXmlFile = Object.values(archive.files)
            .find(file => !file.dir && path.posix.basename(file.name) === 'package.xml');

        if (!packageXmlFile) {
            throw new Error('Invalid package archive: no package.xml found');
        }

        const packageXml = await packageXmlFile.async('nodebuffer');
        const packageXmlJson = XML.parse<{ Package?: { version?: string | number } }>(packageXml);
        const packageVersion = packageXmlJson.Package?.version;
        const apiVersion = `${packageVersion ?? ''}`.replace(/^(\d{2,3})$/, '$1.0');

        if (!/^\d{2,3}\.\d$/.test(apiVersion)) {
            throw new Error(`Invalid API version in package.xml: ${apiVersion}`);
        }

        const packageDir = SalesforcePackage.getArchivePackageDir(packageXmlFile.name);
        const sfPackage = new SalesforcePackage(apiVersion, packageDir);
        sfPackage.manifest.merge(PackageManifest.fromPackageXml(packageXml));

        const destructivePrePath = path.posix.join(packageDir, 'destructiveChangesPre.xml');
        const destructivePre = archive.file(destructivePrePath);
        if (destructivePre) {
            sfPackage.mergeDestructiveChanges(await destructivePre.async('nodebuffer'), 'pre');
        }

        const destructivePostPath = path.posix.join(packageDir, 'destructiveChangesPost.xml');
        const destructivePost = archive.file(destructivePostPath);
        if (destructivePost) {
            sfPackage.mergeDestructiveChanges(await destructivePost.async('nodebuffer'), 'post');
        }

        const manifestComponents = [...sfPackage.manifest.components()];
        for (const archiveFile of Object.values(archive.files)) {
            if (archiveFile.dir || archiveFile.name === packageXmlFile.name ||
                archiveFile.name === destructivePrePath || archiveFile.name === destructivePostPath) {
                continue;
            }

            const packagePath = SalesforcePackage.getArchiveRelativePackagePath(archiveFile.name, packageDir);
            const component = (
                manifestComponents.length === 1
                    ? manifestComponents[0]
                    : {
                        componentType: 'Unknown',
                        componentName: packagePath
                    }
            );

            sfPackage.setPackageData(packagePath, {
                componentType: component.componentType,
                componentName: component.componentName,
                data: await archiveFile.async('nodebuffer')
            });
        }
    ...
    private static getArchivePackageDir(packageXmlPath: string) {
        const packageDir = path.posix.dirname(packageXmlPath.replace(/\/|\\/g, '/'));
        return packageDir === '.' ? '' : packageDir;
    }

    private static getArchiveRelativePackagePath(archivePath: string, packageDir: string) {
        const normalizedArchivePath = archivePath.replace(/\/|\\/g, '/');
        const normalizedPackageDir = packageDir.replace(/\/|\\/g, '/');
        if (normalizedPackageDir && normalizedArchivePath.startsWith(`${normalizedPackageDir}/`)) {
            return normalizedArchivePath.substring(normalizedPackageDir.length + 1);
        }
    ...
    */
    ...
    /**
     * Generates a compressed zip buffer that can be uploaded to Salesforce.
     * Supports both:
     * - `getBuffer(compressionLevel?)`
     * - `getBuffer({ compressionLevel? })`
     * @param compressionOrOptions Compression level (0-9) or options object. Defaults to level 4.
     */
    public async getBuffer(compressionOrOptions: number | SalesforcePackageBufferOptions = 4): Promise<Buffer> {
        const packageZip = await this.generateArchive();
        const compressionLevel = typeof compressionOrOptions === 'number'
            ? compressionOrOptions
            : compressionOrOptions.compressionLevel ?? 4;
        return packageZip.generateAsync({
            type: 'nodebuffer',
            compression: 'DEFLATE',
            compressionOptions: {
    ...
    /**
     * Writes the package zip to a file.
     * Supports both:
     * - `writeToFile(filePath, compressionLevel?)`
     * - `writeToFile(filePath, { compressionLevel? })`
     * @param filePath Path where the zip file will be written.
     * @param compressionOrOptions Compression level (0-9) or options object. Defaults to level 4.
     */
    public async writeToFile(filePath: string, compressionOrOptions: number | SalesforcePackageBufferOptions = 4): Promise<void> {
        return outputFile(filePath, await this.getBuffer(compressionOrOptions));
    }
}
