...
export class DatapackExportDefinitionStore {
    ...
    public clear(scope?: string) {
        delete this.config[scope ?? GLOBAL_SCOPE];
    }
    ...
    public getMatchingKeyFields(item: Exclude<ObjectRef, 'datapackType'>) {
        return this.matchingKeyFields[item.scope ?? GLOBAL_SCOPE]?.[item.objectType] ?? [];
    }

    public isAutoGeneratedMatchingKey(item: Exclude<ObjectRef, 'datapackType'>) {
