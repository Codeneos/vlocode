...
describe('DatapackExporter', () => {
    ...
    it.each([
        ...
        'OmniScript__c'
    ])('resolves namespace-placeholder parent paths in managed embedded object filters for %s', objectType => {
        const definitions = new DatapackExportDefinitionStore();
        const managedDefinitions = jest.requireActual<typeof import('../exportDefinitions')>('../exportDefinitions')
            .DatapackExportDefinitions.omniStudioManaged;
        definitions.load(managedDefinitions.definitions, { scope: managedDefinitions.id });
        const { exporter } = createExporter();
        exporter.definitions = definitions;
        const datapack = {
            id: 'a0M000000000001AAA',
            datapackType: 'OmniScript',
            objectType,
            normalizedObjectType: 'OmniScript__c',
            scope: managedDefinitions.id,
            data: {
                Id: 'a0M000000000001AAA'
            }
        };
    ...
    it('exports an OmniDataTransform item with a matching reference to its parent', async () => {
        const parentId = '0jN000000000001AAA';
        const itemId = '0jO000000000001AAA';
        const definitions = new DatapackExportDefinitionStore();
        const standardDefinitions = DatapackExportDefinitions.omniStudioStandard;
        definitions.load(standardDefinitions.definitions, { scope: standardDefinitions.id });
        const { exporter } = createExporter({
            definitions,
            matchingKeyFieldsByType: {
                OmniDataTransform: ['Name'],
            ...
            describes: {
                ...
                OmniDataTransformItem: {
                    ...
                    fields: [
                        { name: 'Id', referenceTo: [] },
                        { name: 'GlobalKey', referenceTo: [] },
                        { name: 'OmniDataTransformationId', referenceTo: ['OmniDataTransform'] }
                    ],
                    childRelationships: []
                }
            },
            records: [
                { Id: parentId, __type: 'OmniDataTransform', Name: 'ExampleMapper' },
                { Id: itemId, __type: 'OmniDataTransformItem', GlobalKey: 'item-key', OmniDataTransformationId: parentId }
            ]
        });
        ...
        const [result] = await exporter.exportObject({
            ...
            scope: standardDefinitions.id
        }, { maxDepth: 0 });
        const item = result.datapack.OmniDataTransformItem[0];

        expect(item.OmniDataTransformationId).toEqual({
            VlocityDataPackType: 'VlocityMatchingKeyObject',
            VlocityRecordSObjectType: 'OmniDataTransform',
            VlocityMatchingRecordSourceKey: result.datapack.VlocityRecordSourceKey
        });
        expect(item).not.toHaveProperty('VlocityRecordSourceKey');
    });
